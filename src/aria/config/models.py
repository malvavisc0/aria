...
class Chat:
    ...
    repo_id = get_optional_env("CHAT_MODEL", "")
    quantization = get_optional_env("CHAT_MODEL_TYPE", "Q8_0")

    @classmethod
    def get_port(cls) -> int:
        return urllib.parse.urlparse(cls.api_url).port or 7070
...
class Embeddings:
    ...
    repo_id = get_optional_env("EMBEDDINGS_MODEL", "")
    quantization = get_optional_env("EMBEDDINGS_MODEL_TYPE", "Q8_0")

    @classmethod
    def get_port(cls) -> int:
        return urllib.parse.urlparse(cls.api_url).port or 7072


class Vision:
    api_url = get_required_env("VL_OPENAI_API")
    model = get_required_env("VL_MODEL")
    repo_id = get_optional_env("VL_MODEL", "")
    quantization = get_optional_env("VL_MODEL_TYPE", "Q8_0")

    @classmethod
    def get_port(cls) -> int:
        return urllib.parse.urlparse(cls.api_url).port or 7071
