...
from aria.scripts.llama import BINARY_NAMES, download_latest_llama_cpp
...
class TestDownloadLatestLlamaCppUbuntuNoNvcc:
    """Tests for download_latest_llama_cpp() on Ubuntu without nvcc."""

    def test_downloads_prebuilt_binary_success(self, tmp_path: Path):
        """Test that download_latest_llama_cpp downloads pre-built binary on Ubuntu without CUDA."""
        with (
...
"""
    ...
    def test_raises_error_when_no_linux_binary(self, tmp_path: Path):
        """Test that download_latest_llama_cpp raises error when no Linux binary found."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
            patch("aria.scripts.llama._nvcc_available", return_value=False),
...
"""
    ...
    def test_raises_error_when_no_download_url(self, tmp_path: Path):
        """Test that download_latest_llama_cpp raises error when no download URL."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
            patch("aria.scripts.llama._nvcc_available", return_value=False),
...
"""
    ...
    def test_handles_network_error(self, tmp_path: Path):
        """Test that download_latest_llama_cpp handles network errors."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
            patch("aria.scripts.llama._nvcc_available", return_value=False),
...
class TestDownloadLatestLlamaCppUbuntuWithNvcc:
    """Tests for download_latest_llama_cpp() on Ubuntu with nvcc (CUDA)."""

    def test_compiles_from_source_with_cuda(self, tmp_path: Path):
        """Test that download_latest_llama_cpp compiles from source with CUDA on Ubuntu with nvcc."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
...
"""
            ...
            mock_extract_zip.return_value = (
                tmp_path / "source" / "llama.cpp-v1.2.3"
            )
...
class TestDownloadLatestLlamaCppNonUbuntuLinux:
    """Tests for download_latest_llama_cpp() on non-Ubuntu Linux."""

    def test_compiles_from_source_without_cuda(self, tmp_path: Path):
        """Test that download_latest_llama_cpp compiles from source without CUDA on non-Ubuntu Linux."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
            patch("aria.scripts.llama._nvcc_available", return_value=False),
...
"""
    ...
    def test_compiles_from_source_with_cuda_when_nvcc_available(
        self, tmp_path: Path
    ):
        """Test that download_latest_llama_cpp compiles from source with CUDA when nvcc is available."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
            patch("aria.scripts.llama._nvcc_available", return_value=True),
...
class TestDownloadLatestLlamaCppVersionParameter:
    """Tests for download_latest_llama_cpp() with version parameter."""

    def test_downloads_specific_version(self, tmp_path: Path):
        """Test that download_latest_llama_cpp downloads specific version when version is provided."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
            patch("aria.scripts.llama._nvcc_available", return_value=False),
...
"""
    ...
    def test_downloads_latest_when_version_is_latest(self, tmp_path: Path):
        """Test that download_latest_llama_cpp downloads latest when version is 'latest'."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
            patch("aria.scripts.llama._nvcc_available", return_value=False),
...
class TestDownloadLatestLlamaCppBinaryTesting:
    """Tests for binary testing in download_latest_llama_cpp()."""

    def test_tests_binary_after_installation(self, tmp_path: Path):
        """Test that download_latest_llama_cpp tests binary after installation."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
            patch("aria.scripts.llama._nvcc_available", return_value=False),
...
"""
    ...
    def test_handles_binary_test_timeout(self, tmp_path: Path):
        """Test that download_latest_llama_cpp handles binary test timeout."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
            patch("aria.scripts.llama._nvcc_available", return_value=False),
...
"""
    ...
    def test_handles_binary_test_failure(self, tmp_path: Path):
        """Test that download_latest_llama_cpp handles binary test failure."""
        with (
            patch("platform.system", return_value="Linux"),
            patch("builtins.open") as mock_open,
            patch("aria.scripts.llama._nvcc_available", return_value=False),
