...
def test_create_plan(test_db):
    ...
    assert data["result"]["task"] == "Build a web scraper"
...
def test_get_plan(test_db):
    ...
    plan_id = json.loads(create_result)["result"]["plan_id"]

    get_result = get_execution_plan(reason="Testing", execution_id=plan_id)
    data = json.loads(get_result)

    assert data["metadata"]["success"] is True
    assert data["result"]["plan_id"] == plan_id
    assert data["result"]["task"] == "Test task"
...
def test_update_step_status(test_db):
    ...
    plan_id = json.loads(create_result)["result"]["plan_id"]
    step_id = json.loads(create_result)["result"]["steps"][0]["id"]
    ...
    assert data["result"]["result"] == "Worked!"

    # Verify in DB
    get_result = get_execution_plan(reason="Testing", execution_id=plan_id)
    plan_data = json.loads(get_result)["result"]
...
def test_add_step(test_db):
    ...
    plan_id = json.loads(create_result)["result"]["plan_id"]
    step1_id = json.loads(create_result)["result"]["steps"][0]["id"]
    ...
    assert data["metadata"]["success"] is True
    assert data["result"]["inserted_after"] == step1_id
    assert data["result"]["total_steps"] == 3
...
def test_remove_step(test_db):
    ...
    plan_id = json.loads(create_result)["result"]["plan_id"]
    step2_id = json.loads(create_result)["result"]["steps"][1]["id"]
    ...
    assert data["metadata"]["success"] is True
    assert data["result"]["remaining_steps"] == 2
...
def test_replace_step(test_db):
    ...
    assert data["metadata"]["success"] is True
    assert data["result"]["old_description"] == "Old description"
    assert data["result"]["new_description"] == "New description"
...
def test_reorder_steps(test_db):
    ...
    plan_id = json.loads(create_result)["result"]["plan_id"]
    step_ids = [s["id"] for s in json.loads(create_result)["result"]["steps"]]
    ...
    assert data["metadata"]["success"] is True
    assert data["result"]["reordered_steps"][0]["description"] == "Step C"
    assert data["result"]["reordered_steps"][2]["description"] == "Step A"
...
def test_plan_survives_reload(test_db):
    ...
    data = json.loads(get_result)
    assert data["metadata"]["success"] is True
    assert data["result"]["task"] == "Persistent task"
...
def test_multi_agent_isolation(test_db):
    ...
    plan1_id = json.loads(result1)["result"]["plan_id"]
    plan2_id = json.loads(result2)["result"]["plan_id"]
    ...
    assert json.loads(get1)["result"]["task"] == "Agent 1 task"
    assert json.loads(get2)["result"]["task"] == "Agent 2 task"
