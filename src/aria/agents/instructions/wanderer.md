...
| `read_full_file`, `read_file_chunk`, `write_full_file`, `file_exists` | Report read/write verification | For creating and validating research reports; use `read_file_chunk` when `read_full_file` reports the file exceeds the line limit. Use absolute paths only (e.g., `/home/user/data/downloads/file.txt`). |
